...
for (let i = 0; i < personas.length; i++) {
  let personaNueva = {
    ...
    ojos: personas[i].eyeColor,
  };
  personasNuevas.push(personaNueva);
}

 document.getElementById("div").innerHTML = `
  <div style="background-color: ${personasNuevas[0].ojos}" id="persona">
    <img src="${personasNuevas[0].imagen}" />
    <h1>${personasNuevas[0].nombre}</h1>
    <p>${personasNuevas[0].edad}</p>
    <p>${personasNuevas[0].email}</p>
    <p>${personasNuevas[0].direccion}</p>
  </div>`; 

 let eleccion = parseInt(window.prompt("Introduce un índice:"));
if (eleccion >= 0 && eleccion < personasNuevas.length) {
  document.getElementById("div1").innerHTML = `
  <div style="background-color: ${personasNuevas[eleccion].ojos}" id="persona">
    <img src="${personasNuevas[eleccion].imagen}" />
    <h1>${personasNuevas[eleccion].nombre}</h1>
    <p>${personasNuevas[eleccion].edad}</p>
    <p>${personasNuevas[eleccion].email}</p>
    <p>${personasNuevas[eleccion].direccion}</p>
  </div>`;
} else {
  document.getElementById(
    "div1"
  ).innerHTML = `<h1>ERROR!</h1><p>Ereh mu tontoh</p>`;
} 

 let eleccion2 = window.prompt("Introduce una búsqueda:").toLowerCase();
busqueda(eleccion2);
 
function busqueda(texto) {
  parrafo = "";
  let encontrado = false;
  for (let i = 0; i < personasNuevas.length; i++) {
    if (personasNuevas[i].nombre.toLowerCase().includes(texto.toLowerCase())) {
      encontrado = true;
      parrafo += `
    <div style="background-color: ${personasNuevas[i].ojos}" id="persona">
      <img src="${personasNuevas[i].imagen}" />
      <h1>${personasNuevas[i].nombre}</h1>
      <p>${personasNuevas[i].edad}</p>
      <p>${personasNuevas[i].email}</p>
      <p>${personasNuevas[i].direccion}</p>
    </div>`;
    }
  }
  encontrado
...
function busquedaBajoDemanda() {
  let eleccion3 = document.getElementById("entrada").value;
  busqueda(eleccion3);
}