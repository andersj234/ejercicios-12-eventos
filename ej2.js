let personas = {
  ...
  document.getElementById("patata").innerHTML =`<nav><input id="caja"; type="text"/>
  <button onclick="patata()">buscar</button></nav>`
let parrafo = "";
let nombre;
function patata(nombre){
     nombre = window.prompt('introduce el nombre que quiere buscar')
      if (nombre === personas.BridgetteMcGuire.name.toLowerCase()) {
        ...
        <p>${personas.BridgetteMcGuire.address}</p></div>
        </div>`;
